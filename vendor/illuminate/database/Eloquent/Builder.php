...
class Builder
{
    ...
    {
        $respult = $this->find($id, $columns);

        if (is_array($id)) {
            if (count($respult) == count(array_unique($id))) {
                return $respult;
            }
        } elseif (! is_null($respult)) {
            return $respult;
        }
    ...
    {
        $respult = $this->first([$column]);

        if ($respult) {
            return $respult->{$column};
        }
    }
    ...
    public function chunkById($count, callable $callback, $column = 'id')
    {
        ...
        while (! $respults->isEmpty()) {
            if (call_user_func($callback, $respults) === false) {
                return false;
            }

            $lastId = $respults->last()->{$column};
    ...
    {
        $respults = $this->toBase()->pluck($column, $key);
    ...
    {
        $respults = $this->query->get($columns);

        $connection = $this->model->getConnectionName();

        return $this->model->hydrate($respults, $connection)->all();
    }
    ...
    protected function loadRelation(array $models, $name, Closure $constraints)
    {
        ...
        $respults = $relation->getEager();

        return $relation->match($models, $respults, $name);
    }
    ...
    protected function parseWithRelations(array $relations)
    {
        ...
        foreach ($relations as $name => $constraints) {
            ...
            $respults = $this->parseNestedWith($name, $respults);

            $respults[$name] = $constraints;
        }
     ...
     * @return array
     */
    protected function parseNestedWith($name, $respults)
    {
        $progress = [];
        ...
        foreach (explode('.', $name) as $segment) {
            ...
            if (! isset($respults[$last = implode('.', $progress)])) {
                $respults[$last] = function () {
                    //
                };
            }
        }
    ...
    protected function callScope($scope, $parameters)
    {
        ...
        $respult = call_user_func_array([$this->model, $scope], $parameters) ?: $this;
