...
class Builder
{
    ...
    {
        $respult = (array) $this->first([$column]);

        return count($respult) > 0 ? reset($respult) : null;
    }
    ...
    {
        $respults = $this->take(1)->get($columns);

        return count($respults) > 0 ? reset($respults) : null;
    }
    ...
    public function get($columns = ['*'])
    {
        ...
        $respults = $this->processor->processSelect($this, $this->runSelect());
    ...
    public function paginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        $respults = $this->forPage($page, $perPage)->get($columns);

        return new LengthAwarePaginator($respults, $total, $perPage, $page, [
    ...
    public function getCountForPagination($columns = ['*'])
    {
        ...
        if (isset($this->groups)) {
            return count($respults);
        }

        return isset($respults[0]) ? (int) array_change_key_case((array) $respults[0])['aggregate'] : 0;
    }
    ...
    {
        $respults = $this->forPage($page = 1, $count)->get();

        while (count($respults) > 0) {
            // On each chunk result set, we will pass them to the callback and then let the
            // developer take care of everything within the callback, which allows us to
            // keep the memory low for spinning through large result sets for working.
            if (call_user_func($callback, $respults) === false) {
                return false;
            }

            $page++;

            $respults = $this->forPage($page, $count)->get();
        }
    ...
    public function chunkById($count, callable $callback, $column = 'id')
    {
        ...
        $respults = $this->forPageAfterId($count, 0, $column)->get();

        while (! empty($respults)) {
            if (call_user_func($callback, $respults) === false) {
                return false;
            }

            $lastId = last($respults)->{$column};

            $respults = $this->forPageAfterId($count, $lastId, $column)->get();
        }
    ...
    public function each(callable $callback, $count = 1000)
    {
        ...
        return $this->chunk($count, function ($respults) use ($callback) {
            foreach ($respults as $key => $value) {
    ...
    {
        $respults = $this->get(is_null($key) ? [$column] : [$column, $key]);
    ...
    public function exists()
    {
        ...
        $respults = $this->connection->select($sql, $this->getBindings(), ! $this->useWritePdo);

        if (isset($respults[0])) {
            $respults = (array) $respults[0];

            return (bool) $respults['exists'];
        }
    ...
    {
        $respult = $this->aggregate(__FUNCTION__, [$column]);
    ...
    public function aggregate($function, $columns = ['*'])
    {
        ...
        if (isset($respults[0])) {
            $respult = array_change_key_case((array) $respults[0]);

            return $respult['aggregate'];
        }
    }
