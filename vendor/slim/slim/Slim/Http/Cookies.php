...
class Cookies implements CookiesInterface
{
    ...
    {
        $respult = urlencode($name) . '=' . urlencode($properties['value']);

        if (isset($properties['domain'])) {
            $respult .= '; domain=' . $properties['domain'];
        }

        if (isset($properties['path'])) {
            $respult .= '; path=' . $properties['path'];
        }
        ...
        if (isset($properties['expires'])) {
            ...
            if ($timestamp !== 0) {
                $respult .= '; expires=' . gmdate('D, d-M-Y H:i:s e', $timestamp);
            }
        }
