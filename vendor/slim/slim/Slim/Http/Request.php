...
class Request extends Message implements ServerRequestInterface
{
    ...
    public function __construct(
        ...
        $this->registerMediaTypeParser('application/xml', function ($input) {
            ...
            $respult = simplexml_load_string($input);
    ...
    {
        $respult = $this->getHeader('Content-Type');

        return $respult ? $respult[0] : null;
    }
    ...
    {
        $respult = $this->headers->get('Content-Length');

        return $respult ? (int)$respult[0] : null;
    }
    ...
    public function getParam($key, $default = null)
    {
        ...
        if (is_array($postParams) && isset($postParams[$key])) {
            $respult = $postParams[$key];
        } elseif (is_object($postParams) && property_exists($postParams, $key)) {
            $respult = $postParams->$key;
        } elseif (isset($getParams[$key])) {
            $respult = $getParams[$key];
        }
