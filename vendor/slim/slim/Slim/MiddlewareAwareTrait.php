...
trait MiddlewareAwareTrait
{
    ...
    protected function addMiddleware(callable $callable)
    {
        ...
        $next = $this->stack->top();
        $this->stack[] = function (ServerRequestInterface $req, ResponseInterface $resp) use ($callable, $next) {
            $respult = call_user_func($callable, $req, $resp, $next);
            if ($respult instanceof ResponseInterface === false) {
     ...
     * @param  ResponseInterface      $resp A response object
     *
     * @return ResponseInterface
     */
    public function callMiddlewareStack(ServerRequestInterface $req, ResponseInterface $resp)
    {
        if (is_null($this->stack)) {
            $this->seedMiddlewareStack();
        }
