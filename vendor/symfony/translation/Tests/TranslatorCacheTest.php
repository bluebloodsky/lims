...
class TranslatorCacheTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $respource = $this->getMock('Symfony\Component\Config\Resource\SelfCheckingResourceInterface');
        $loader = $this->getMock('Symfony\Component\Translation\Loader\LoaderInterface');
        $respource->method('isFresh')->will($this->returnValue(false));
        $loader
            ->expects($this->exactly(2))
            ->method('load')
            ->will($this->returnValue($this->getCatalogue('fr', array(), array($respource))));
    ...
    protected function getCatalogue($locale, $messages, $respources = array())
    {
        $catalogue = new MessageCatalogue($locale);
        foreach ($messages as $key => $translation) {
            $catalogue->set($key, $translation);
        }
        foreach ($respources as $respource) {
            $catalogue->addResource($respource);
        }
