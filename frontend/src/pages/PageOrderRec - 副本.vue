<template>
  <div>
    ...
    <div class="nav">
      <ul class="left-info">
        <li :class="{done:done_page>index,current:cur_page==index}" v-for="(page,index) in pages">
          <a @click="cur_page=index" v-if="done_page>index">{{page}}</a>
          <span v-else>{{page}}</span>
        </li>
      </ul>
      <div class="right-btn">
        <button>流程复制</button>
        <button v-if="cur_page == 2">下单</button>
        <button @click="cur_page++;done_page++">保存</button>
      </div>
    </div>
